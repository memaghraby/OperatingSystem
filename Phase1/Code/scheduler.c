...
int main(int argc, char * argv[])
{
    ...
    while(1){
        ...
        if (rec_val != -1){ //chech if a new process was received
            int x = getClk();
            //printf("\n----------------------\nAt time %d process with id %d arrived \n", x, message.mtext.id);
            if(choice == 1){
                enqueue(message.mtext, message.mtext.priority);
            }
            else if(choice == 2){
                enqueue(message.mtext, message.mtext.runningtime);
                if(runningProcess){
                    int remainingtime = (runningProcess->data.runningtime - (getClk() - runningProcess->data.starttime - atoi((char *)sharedWaitedTime)));
                    if((remainingtime != 0) && (message.mtext.runningtime < remainingtime)){
                        kill(pid, SIGSTOP);
